...
Фронт подключён к бэкенду: регистрация, вход, анкета, рейтинг, загрузка
выписки и Telegram ходят в API. Ниже — контракты и то, что важно знать.

## 0. Структура

Проект собирается через Vite, исходники в `src/` — так, как этого требуют
импорты в `App.jsx`:
...
src/
  ...
  pages/                 экраны, включая StatementUploadPage и TelegramAnalysisPage
  theme/config.js        параметры оформления
  utils/                 surveyToAccount (анкета -> поля API), recommendations
```
...
// report.telegram_delta   — поправка Telegram −25..+25 (НЕ балл)
// report.telegram_risk    — 'low' | 'medium' | 'high' | 'insufficient_data'
// report.telegram_comment — объяснение поправки на русском
```
...
Telegram устроен иначе двух других: это **поправка**, а не балл. Итог =
`clamp(среднее(анкета, выписка) + telegram_delta, 0, 100)`. Показывать её
надо со знаком и отдельно от базовых веток.
